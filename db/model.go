...
func SetupDB(ctx context.Context) error {
	var (
		dbHost     = env.GetString("JUMP_DB_HOST")
		dbPort     = env.GetString("JUMP_DB_PORT")
		dbName     = env.GetString("JUMP_DB_NAME")
		dbUsername = env.GetString("JUMP_DB_USERNAME")
		dbPassword = env.GetString("JUMP_DB_PASSWORD")
		dbTraceLog = env.GetBool("JUMP_DB_TRACE_LOG")
	)
